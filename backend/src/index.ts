...
process.on("uncaughtException", (error) => {
  console.error("❌ Erreur non capturée (uncaughtException):", error);
  console.error("Stack trace:", error.stack);
  // Ne pas process.exit(1) pour éviter la boucle de redémarrage de nodemon
});
...
const startServer = async () => {
  try {
    ...
    // Gestion des erreurs du serveur HTTP (ex: port déjà utilisé)
    server.on("error", (error: NodeJS.ErrnoException) => {
      if (error.code === "EADDRINUSE") {
        console.error(`❌ Le port ${PORT} est déjà utilisé.`);
        console.error("   Arrêtez les autres instances ou changez le port dans .env (PORT=XXXX)");
      } else {
